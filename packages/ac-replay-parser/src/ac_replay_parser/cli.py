...
def output_car_frame(frame: CarFrame) -> str:
    ...
    for wheel in frame.wheels:
        add_f32(
            (wheel.static_position.x, wheel.static_position.y, wheel.static_position.z)
        )
    for wheel in frame.wheels:
        add_f16(
            (wheel.static_rotation.x, wheel.static_rotation.y, wheel.static_rotation.z)
        )
    for wheel in frame.wheels:
...
def output_path_for_car(
    ...
    if preferred_output is None or preferred_output.endswith(("/", "\\")):
        ...
        suffix = (
            f"_{driver_name}" if target_driver_name is None and num_cars > 1 else ""
        )
...
def parse_replay(
    ...
    if replay.csp_data_offset is not None:
        ...
        if (
            target_driver_name is not None
            and target_driver_name not in replay.driver_names
        ):
    ...
    for car in replay.cars:
        ...
        if (
            target_driver_name is not None
            and target_driver_name != car_header.driver_name
        ):
            continue
