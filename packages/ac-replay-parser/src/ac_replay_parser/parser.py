    ...
    try:
        for tag, payload_offset in iter_csp_chunks(stream, csp_offset, file_size):
            ...
            try:
                ...
                raise ReplayError(
                    f"EXT_PERCAR decompression failed: {error}"
                ) from error
...
@overload
def unpack_values(
    data: bytes, offset: int, fmt: Literal["eee"]
) -> tuple[float, float, float]: ...


@overload
def unpack_values(
    data: bytes, offset: int, fmt: Literal["fff"]
) -> tuple[float, float, float]: ...
...
@overload
def unpack_values(
    data: bytes, offset: int, fmt: Literal["BBBB"]
) -> tuple[int, int, int, int]: ...


@overload
def unpack_values(
    data: bytes, offset: int, fmt: Literal["III"]
) -> tuple[int, int, int]: ...
...
def read_car_frames(
    stream: BinaryIO, car_header: CarHeader
    ...
    for car_index in range(header.num_cars):
        ...
        if csp_offset is not None:
            ...
            if raw_extra is not None:
                ...
                extra_frames = tuple(
                    parse_extra_frame(
                        extra_data[offset : offset + frame_size], extra_version
                    )
                    for offset in range(
                        0, frame_size * car_header.num_frames, frame_size
                    )
                )
        parsed_cars.append(
            ParsedCar(
