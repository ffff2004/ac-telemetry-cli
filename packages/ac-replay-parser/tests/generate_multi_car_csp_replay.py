...
def make_car_frame(car_index: int, frame_index: int) -> bytes:
	...
	for wheel_index, offset in enumerate(range(92, 140, 12)):
		struct.pack_into(
			"<fff", data, offset, wheel_index + 0.4, seed + 0.5, -0.6
		)
...
def make_extra_frames(version: int, car_index: int, frame_count: int) -> bytes:
	...
	for frame_index in range(frame_count):
		...
		status = (car_index % 5) | (1 << 3) | (1 << 4) | (1 << 6) | (1 << 10) | (1 << 15)
...
def main() -> int:
	...
	if args.output.exists() and not args.force:
		raise SystemExit(f'Output file "{args.output}" already exists; pass --force to overwrite it')
